...
export function ModelPerformanceTable() {
	...
	const [selectedDataset, setSelectedDataset] =
		React.useState<string>('Celeb-DF');
	...
	const filteredData = useMemo(() => {
		return initialData.filter(
			(item) => item.datasetName === selectedDataset,
		);
						...
						<TableBody className="**:data-[slot=table-cell]:first:w-8">
							{table.getRowModel().rows?.length ? (
								(() => {
									...
									let prevDataset: string | null = null,
										prevAlteration: string | null = null,
										prevVariation: string | null = null;
									let datasetStart = 0,
										alterationStart = 0,
										variationStart = 0;
									// Calculate rowSpans for each group
									const datasetGroups: Record<
										string,
										{ start: number; span: number }
									> = {};
									const alterationGroups: Record<
										string,
										{ start: number; span: number }
									> = {};
									...
									rows.forEach((row, i) => {
										...
										if (
											...
											if (prevAlteration !== null) {
												alterationGroups[
													`${prevDataset}-${prevAlteration}-${alterationStart}`
												].span = i - alterationStart;
											}
											alterationGroups[`${d}-${a}-${i}`] =
												{ start: i, span: 1 };
											alterationStart = i;
										}
										// Variation grouping
										if (
											...
											if (prevVariation !== null) {
												variationGroups[
													`${prevDataset}-${prevAlteration}-${prevVariation}-${variationStart}`
												].span = i - variationStart;
											}
											variationGroups[
												`${d}-${a}-${v}-${i}`
											] = { start: i, span: 1 };
									...
									if (rows.length) {
										datasetGroups[
											`${prevDataset}-${datasetStart}`
										].span = rows.length - datasetStart;
										alterationGroups[
											`${prevDataset}-${prevAlteration}-${alterationStart}`
										].span = rows.length - alterationStart;
										variationGroups[
											`${prevDataset}-${prevAlteration}-${prevVariation}-${variationStart}`
										].span = rows.length - variationStart;
									}
