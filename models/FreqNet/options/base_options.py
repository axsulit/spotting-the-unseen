...
class BaseOptions():
    """
    Base class for handling command-line options used across training and testing.
    Initializes, parses, and stores experiment configurations.
    """
    ...
    def initialize(self, parser):
        """Define the common options used in training and testing."""
    ...
    def gather_options(self):
        """Initialize parser and gather options from the command line."""
    ...
    def print_options(self, opt):
        """Print and save options to disk."""
        message = ''
        message += '----------------- Options ---------------\n'
    ...
    def parse(self, print_options=True):
        """
        Parse options, handle GPU setup, process list inputs, and return final options.

        Args:
            print_options (bool): Whether to print and save options.
        ...
        # Only set CUDA device if available and GPU was requested
        if len(opt.gpu_ids) > 0 and torch.cuda.is_available():
            torch.cuda.set_device(opt.gpu_ids[0])
        elif len(opt.gpu_ids) > 0:
            print("[Warning] GPU requested but not available. Running on CPU instead.")
