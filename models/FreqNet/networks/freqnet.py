...
class BasicBlock(nn.Module):
    """Residual block for small architectures."""
...
class Bottleneck(nn.Module):
    """Bottleneck residual block for deep architectures."""
    expansion = 4
...
class FreqNet(nn.Module):
    """
    Frequency-based deep neural network with hybrid spatial-frequency feature transformations.

    Applies Fourier-based operations for high-frequency removal and frequency convolution layers (FCL).
    Inherits from ResNet structure.
    """

    def __init__(self, block=Bottleneck, layers=[3, 4], num_classes=1, zero_init_residual=False, device='cpu'):
        ...
        self.weight1 = nn.Parameter(torch.randn((64, 3, 1, 1), device=self.device))
        self.bias1 = nn.Parameter(torch.randn((64,), device=self.device))
        ...
        self.weight2 = nn.Parameter(torch.randn((64, 64, 1, 1), device=self.device))
        self.bias2 = nn.Parameter(torch.randn((64,), device=self.device))
        ...
        self.weight3 = nn.Parameter(torch.randn((256, 256, 1, 1), device=self.device))
        self.bias3 = nn.Parameter(torch.randn((256,), device=self.device))
        ...
        self.weight4 = nn.Parameter(torch.randn((256, 256, 1, 1), device=self.device))
        self.bias4 = nn.Parameter(torch.randn((256,), device=self.device))
    ...
    def hfreqWH(self, x, scale):
        """Apply high-frequency removal in spatial domain via 2D FFT."""
    ...
    def hfreqC(self, x, scale):
        """Apply high-frequency removal in channel domain via 1D FFT."""
...
def freqnet(**kwargs):
    """Helper function to instantiate FreqNet model."""
