"""Multiple Attention Transformer (MAT) model and related components.

This module implements the MAT architecture and its supporting components for deepfake detection
and related tasks. It includes attention map extraction, attention pooling, texture enhancement,
auxiliary loss functions, and the main MAT model class, supporting both Xception and EfficientNet
backbones. All classes and functions are documented according to PEP 257 guidelines.
"""
...
class AttentionMap(nn.Module):
    """Extracts attention maps from feature maps using convolutional layers.

    Args:
        in_channels (int): Number of input channels.
        out_channels (int): Number of output attention maps.
    """
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output attention maps.
        """
...
class AttentionPooling(nn.Module):
    """Performs attention-based pooling on feature maps."""
    def __init__(self):
        super().__init__()
    ...
    def forward(self, features, attentions, norm=2):
        ...
        Args:
            features (torch.Tensor): Feature maps.
            attentions (torch.Tensor): Attention maps.
            norm (int, optional): Normalization type. Defaults to 2.
        Returns:
            torch.Tensor: Pooled feature matrix.
        """
        H, W = features.size()[-2:]
...
class Texture_Enhance_v1(nn.Module):
    """Texture enhancement module (version 1) for feature maps.

    Args:
        num_features (int): Number of input features.
        num_attentions (int): Number of attention maps.
    """
    ...
    def forward(self, feature_maps, attention_maps=(1, 1)):
        """Forward pass for texture enhancement.

        Args:
            feature_maps (torch.Tensor): Input feature maps.
            attention_maps (tuple or torch.Tensor): Attention map size or tensor.
        Returns:
            tuple: Enhanced feature maps and downsampled feature maps.
        """
...
class Texture_Enhance_v2(nn.Module):
    """Texture enhancement module (version 2) for feature maps.
    ...
    def cat(self, a, b):
        """Concatenate tensors along the channel dimension for grouped convolutions.
    ...
    def forward(self, feature_maps, attention_maps=(1, 1)):
        """Forward pass for texture enhancement (v2).
...
class Auxiliary_Loss_v2(nn.Module):
    """Auxiliary loss function (version 2) for feature matrix regularization.

    Args:
        M (int): Number of attention maps.
        N (int): Feature dimension.
        C (int): Number of classes.
        alpha (float, optional): Center update rate. Defaults to 0.05.
        margin (float, optional): Margin for inter-class loss. Defaults to 1.
        inner_margin (list, optional): Inner margin values. Defaults to [0.1, 5].
    """
    ...
    def forward(self, feature_map_d, attentions, y):
        """Forward pass for auxiliary loss computation.

        Args:
            feature_map_d (torch.Tensor): Downsampled feature maps.
            attentions (torch.Tensor): Attention maps.
            y (torch.Tensor): Ground truth labels.
        Returns:
            tuple: (Total auxiliary loss, feature matrix)
        """
...
class Auxiliary_Loss_v1(nn.Module):
    """Auxiliary loss function (version 1) for feature matrix regularization.
    ...
    Args:
        ...
        alpha (float, optional): Center update rate. Defaults to 0.05.
        margin (float, optional): Margin for inter-class loss. Defaults to 1.
        inner_margin (list, optional): Inner margin values. Defaults to [0.01, 0.02].
    """
    ...
    def forward(self, feature_map_d, attentions, y):
        """Forward pass for auxiliary loss computation (v1).
...
class MAT(nn.Module):
    """Multiple Attention Transformer (MAT) model for deepfake detection.

    This class implements the MAT model, supporting both Xception and EfficientNet backbones.

    Args:
        net (str, optional): Backbone network name. Defaults to 'xception'.
        feature_layer (str, optional): Feature layer name. Defaults to 'b3'.
        attention_layer (str, optional): Attention layer name. Defaults to 'final'.
        num_classes (int, optional): Number of output classes. Defaults to 2.
        M (int, optional): Number of attention maps. Defaults to 8.
        mid_dims (int, optional): Intermediate feature dimension. Defaults to 256.
        dropout_rate (float, optional): Dropout rate. Defaults to 0.5.
        drop_final_rate (float, optional): Dropout rate for final layer. Defaults to 0.5.
        pretrained (bool or str, optional): Whether to use pretrained weights. Defaults to False.
        alpha (float, optional): Center update rate for auxiliary loss. Defaults to 0.05.
        size (tuple, optional): Input image size. Defaults to (380, 380).
        margin (float, optional): Margin for auxiliary loss. Defaults to 1.
        inner_margin (list, optional): Inner margin values. Defaults to [0.01, 0.02].
    """
    ...
    def train_batch(self, x, y, jump_aux=False, drop_final=False):
        """Train batch step for MAT model.

        Args:
            x (torch.Tensor): Input images.
            y (torch.Tensor): Ground truth labels.
            jump_aux (bool, optional): Whether to skip auxiliary loss. Defaults to False.
            drop_final (bool, optional): Whether to drop final features. Defaults to False.
        Returns:
            dict: Dictionary containing losses, logits, and intermediate features.
        """
        layers = self.net(x)
    ...
    def forward(self, x, y=0, train_batch=False, AG=None):
        ...
        Args:
            x (torch.Tensor): Input images.
            y (torch.Tensor, optional): Ground truth labels. Defaults to 0.
            train_batch (bool, optional): Whether to run in training mode. Defaults to False.
            AG (object, optional): Attention-guided data augmentation. Defaults to None.
        Returns:
            torch.Tensor or dict: Model output logits or dictionary of losses/features.
        """
        if train_batch:
            if AG is None:
...
def load_state(net, ckpt):
    """Load model state dictionary with shape checking.

    Args:
        net (nn.Module): Model to load state into.
        ckpt (dict): Checkpoint state dictionary.
    """
    sd = net.state_dict()
    nd = {}
    for i in ckpt:
...
class netrunc(nn.Module):
    """Truncated network for feature extraction with optional texture enhancement.
    ...
    def forward(self, x):
        """Forward pass for truncated network.
