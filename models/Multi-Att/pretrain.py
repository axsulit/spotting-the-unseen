"""Pretraining module for the multiple attention model.

This module provides functions for pretraining the model backbone on the deepfake
detection task using distributed training across multiple GPUs.
"""
...
def main_worker(local_rank, world_size, rank_offset, config):
    ...
    dist.init_process_group(backend='nccl', init_method='file://%s/.pytorch_distribute' % os.getcwd(),
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        run(logs=logs, data_loader=train_loader, net=net, optimizer=optimizer, local_rank=local_rank,
            config=config, phase='train')
        run(logs=logs, data_loader=validate_loader, net=net, optimizer=optimizer, local_rank=local_rank,
...
def run(logs, data_loader, net, optimizer, local_rank, config, phase='train'):
    ...
    Args:
        ...
        local_rank (int): Local rank of the current GPU.
        config (train_config): Training configuration object.
        phase (str, optional): Current phase ('train' or 'valid'). Defaults to 'train'.
    """
    if local_rank == 0:
        print('start ', phase)
    recorder = {}
...
if __name__ == "__main__":
    ...
    for feature in range(2, 5):
        ...
        Config = train_config(name, ['ff-all-c23', 'efficientnet-b4'], attention_layer='',
                            feature_layer=feature_layer, epochs=20, batch_size=10, augment='augment1')
        Config.mkdirs()
